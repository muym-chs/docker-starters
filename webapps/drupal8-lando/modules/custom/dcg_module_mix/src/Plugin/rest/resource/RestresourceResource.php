...
namespace Drupal\dcg_module_mix\Plugin\rest\resource;
...
/**
 * Represents RestResource records as resources.
 *
 * @RestResource (
 *   id = "dcg_module_mix_restresource",
 *   label = @Translation("RestResource"),
 *   uri_paths = {
 *     "canonical" = "/api/dcg-module-mix-restresource/{id}",
 *     "https://www.drupal.org/link-relations/create" = "/api/dcg-module-mix-restresource"
 *   }
 * )
 *
 * @DCG
 ...
 */
class RestresourceResource extends ResourceBase implements DependentPluginInterface {
  ...
  public function post($data) {
    ...
    $id = $this->dbConnection->insert('dcg_module_mix_restresource')
      ->fields($data)
      ->execute();

    $this->logger->notice('New restresource record has been created.');
  ...
  public function delete($id) {
    ...
    $this->dbConnection->delete('dcg_module_mix_restresource')
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('RestResource record @id has been deleted.', ['@id' => $id]);
  ...
  protected function loadRecord($id) {
    $record = $this->dbConnection->query('SELECT * FROM {dcg_module_mix_restresource} WHERE id = :id', [':id' => $id])->fetchAssoc();
    if (!$record) {
  ...
  protected function updateRecord($id, array $record) {
    ...
    $this->dbConnection->update('dcg_module_mix_restresource')
      ->fields($record)
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('RestResource record @id has been updated.', ['@id' => $id]);
